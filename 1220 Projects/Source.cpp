/*Author: Tom Lowry
Purpose: This file gives the code that reverses a string passed to it between two given indexes
Date: 23 Jun 2018
...
int main(int argc, char** argv) {
	...
	isDigit1 = isInteger(strNumCheck1);
	isDigit2 = isInteger(strNumCheck2);
	...
	cout << "Reversing \"" << strcopy << "\" from position " << front << " to postion " << rear << " yields \"" << argv[1] << "\"." << endl;
	return 0;
}
...
void reverse(char *front, char *rear) {
	...
	//Changing front char with rear and then incrementing the front and decrementing the rear
	while (front <= rear) {
		temp = *front;
		*front = *rear;
		*rear = temp;
